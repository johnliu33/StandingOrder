...
- (void)willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    NSLog(@"Y center:%f",CGRectGetMidY( self.view.bounds ));
	[UIView animateWithDuration:duration
					 animations:^{
						 [av sizeToFit];
						 av.center = CGPointMake( CGRectGetMidX( self.view.bounds ), CGRectGetMidY( self.view.bounds )-150);;
...
const CGFloat kTSAlertView_TopMargin	= 16.0;//default 16.0
...
- (UITextView*) messageTextView
{
	if ( _messageTextView == nil )
	{
		...
		_messageTextView.editable = YES;//NO;
...
- (void) show
{
	...
	TSAlertOverlayWindow* ow = [[TSAlertOverlayWindow alloc] initWithFrame: [UIScreen mainScreen].bounds];//[UIScreen mainScreen].bounds//CGRectMake(0, 0, 320, 240)
	ow.alpha = 0.0;
	...
	[self sizeToFit];
	self.center = CGPointMake( CGRectGetMidX( avc.view.bounds ), CGRectGetMidY( avc.view.bounds )+20 );;
	self.frame = CGRectIntegral( self.frame );
	[self pulse];
...
- (CGSize) recalcSizeAndLayout: (BOOL) layout
{
	...
	CGSize  titleLabelSize = CGSizeMake(0, 0);//[self titleLabelSize];
	CGSize  messageViewSize = [self messageLabelSize];
	CGSize  inputTextFieldSize = [self inputTextFieldSize];
    NSLog(@"inputHeight:%f",inputTextFieldSize.height);
	...
	CGFloat totalHeight = kTSAlertView_TopMargin + titleLabelSize.height + kTSAlertView_RowMargin + messageViewSize.height +  kTSAlertView_RowMargin + buttonsAreaSize.height + kTSAlertView_BottomMargin;//inputRowHeight +
	...
	if ( layout )
	{
		...
		if ( self.title != nil )
		{
			...
			[self addSubview: self.titleLabel];//Label Height changehere
