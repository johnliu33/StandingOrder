...
@synthesize _bibleDict;

- (void)initBibleDict
{
    _bibleDict = [NSDictionary dictionaryWithObjectsAndKeys:                             
                  @"1",@"創世記",
                  @"1",@"創",
                  @"2",@"出埃及記",
                  @"2",@"出埃及",                  
...
- (void)findVerse:(NSString *)myfindVerse
{
    ...
    /*if(_rRange.location != NSNotFound)
        Se_ = [NSString stringWithString:[myfindVerse1 substringToIndex:_rRange.location]];
    else if(_rSe.location != NSNotFound)
        Se_ = [NSString stringWithString:[myfindVerse1 substringToIndex:_rSe.location]];
    else {
        return;
    }*/
...
- (void)findChapter:(NSString *)myfindVerse
{
    NSRange _rBook;
    NSRange _rCh;
    //NSRange _rSe;
    ...
    int _iC =[self convertChineseNumber:Ch_];
    int _iS = 1;//[self convertChineseNumber:Se_];
    ...
    NSString *sql = [NSString stringWithFormat:@"SELECT book,chapter,section,column,verse FROM hant where book = %i and chapter = %i and section between %i and %i",_iBook,_iC,_iS,_iS+50];
    NSLog(@"sql:%@",sql);
    sqlite3_stmt *statement = [app.dbHelper executeQuery:sql];
...
-(void)findKeywords:(NSString *)myfindString{
    ...
    NSString *sql = [NSString stringWithFormat:@"SELECT book,chapter,section,column,verse FROM hant where verse like '%%%@%%'",myfindString];
    //NSLog(@"sql:%@",sql);
    sqlite3_stmt *statement = [app.dbHelper executeQuery:sql];
...
- (NSString *)isItShortVerseSearch:(NSString *)findText
{
    ...
    for(NSString *_bookNames in _bibleKeys)
    {
        ...
        if(_find.location != NSNotFound)
        {
            _iBook =  [[_bibleDict objectForKey:_bookNames] intValue];
            [_fixBookName setString:_bookNames];
            if([_fixBookName isEqualToString:@"書"] && [findText rangeOfString:@"書"].location != 0)
            {
                continue;
            }
            
            if([_fixBookName isEqualToString:@"羅"] && [findText rangeOfString:@"撒羅尼"].location != NSNotFound)
            {
                continue;
            }
            
            if([_fixBookName isEqualToString:@"得"] && [findText rangeOfString:@"彼得"].location != NSNotFound)
            {
                continue;
            }
            
            if([_fixBookName isEqualToString:@"多"] && [findText rangeOfString:@"哥林多"].location != NSNotFound)
            {
                continue;
            }
...
- (BOOL)isItChapterSearch:(NSString *)findText
{
    NSRange _rCh = [findText rangeOfString:@"章"];
    //NSRange _rHymn = [findText rangeOfString:@"篇"];
    //NSCharacterSet* charOfDiv = [NSCharacterSet characterSetWithCharactersInString:@"至及和到節"];
...
- (void)tableWithSearch
{
    ...
    else if([self isItChapterSearch:_text]) {
        NSLog(@"Find Chapter!!");
        [self findChapter:_text];
    }
    else
