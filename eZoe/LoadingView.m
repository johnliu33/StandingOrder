...
{
    CGRect bounds = [aSuperview bounds];
    CGFloat _width;
    CGFloat _height;
    ...
    CGRect _frame = CGRectMake(bounds.size.width/2-_width/2, bounds.size.height/2-_height/2, _width, _height);
	LoadingView *loadingView =
		[[[LoadingView alloc] initWithFrame:_frame] autorelease];//[aSuperview bounds]
	if (!loadingView)
	{
		return nil;
	}
	...
	UILabel *loadingLabel =
		...
		autorelease];
	loadingLabel.text = NSLocalizedString(@"載入門市中", nil);
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
        activityIndicatorRect.origin.y = floor(0.7 * (loadingView.frame.size.height - totalHeight));
    else
        activityIndicatorRect.origin.y = floor(2.2 * (loadingView.frame.size.height - totalHeight));

    activityIndicatorView.frame = activityIndicatorRect;
    
	labelFrame.origin.x = floor(0.5 * (loadingView.frame.size.width - DEFAULT_LABEL_WIDTH));
	labelFrame.origin.y = activityIndicatorView.frame.origin.y + activityIndicatorView.frame.size.height;//floor(0.5 * (loadingView.frame.size.height - totalHeight));
...
- (void)drawRect:(CGRect)rect
{
	...
	const CGFloat ROUND_RECT_CORNER_RADIUS = 15.0;
