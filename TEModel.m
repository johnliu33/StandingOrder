...
- (void)notifyDidUpdateProgress:(long long)progress total:(long long)total {
    if ([self.delegate respondsToSelector:@selector(model:didUpdateProgress:total:)]) {
        [self notifyOnMainThread:^{
            [self.delegate model:self didUpdateProgress:progress total:total];
        }];
    }
}
